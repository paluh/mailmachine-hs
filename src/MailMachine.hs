...
module MailMachine
       (enqueue, mailQueue, FromAddress, ReceiversAddresses, Subject,
        MailQueue)
       where
...
import MailMachine.Types
       (FromAddress, MailQueue, ReceiversAddresses, connection, mailQueue,
        queueName, Subject, BodyPlain, BodyHtml)
import Network.Mail.Mime
       (Address, Mail, addressEmail, addressName, emptyMail, htmlPart,
        mailHeaders, mailParts, mailTo, plainPart, renderMail')

enqueue :: MailQueue -> FromAddress -> ReceiversAddresses -> Subject -> BodyPlain -> BodyHtml -> IO (Either Reply Integer)
enqueue mq from to subject plainBody htmlBody = do
    let msg =
            buildMessage from to subject plainBody htmlBody
        qName =
            encodeUtf8 . queueName $ mq
        conn =
            connection mq
    raw <- renderMail' msg
    let serializedMessage =
            serializeMessage from to raw
    runRedis conn $
      rpush qName [B.concat . L.toChunks $ serializedMessage]
